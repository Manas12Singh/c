...
short lencompare(struct Node *b1, struct Node *b2)
{
    int len1 = 0, len2 = 0;
    while (b1)
        len1++, b1 = b1->next;
...
short compare(struct Node *b1, struct Node *b2)
{
    if (b1 == NULL)
        return 0;
...
short compareBigInteger(BigInteger *b1, BigInteger *b2)
{
    if (b1->sign != b2->sign)
        return b1->sign;
...
BigInteger *addBigInteger(BigInteger *b1, BigInteger *b2)
{
    ...
    if (b1->sign == b2->sign)
        return add(b1, b2);
    short l = lencompare(b1->head, b2->head);
    if (l == 1)
        return sub(b1, b2);
    else if (l == -1)
        return sub(b2, b1);
